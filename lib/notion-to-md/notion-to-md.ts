...
export class NotionToMarkdown {
   ...
   */
  async pageToMarkdown(id: string, totalPage: number | null = null): Promise<MdBlock[]> {
      ...
      case 'image':
        {
          ...
          if (image_type === 'external') return md.image(image_caption_plain, blockContent.external.url);
          if (image_type === 'file') return md.image(image_caption_plain, blockContent.file.url);
        }
        break;
      ...
      case 'pdf':
        {
          ...
          if (blockContent) {
            ...
            if (file_type === 'external') return md.link('image', blockContent.external.url);
            if (file_type === 'file') return md.link('image', blockContent.file.url);
          }
        }
        break;
